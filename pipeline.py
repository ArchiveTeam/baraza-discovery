    ...
    def realize(self, item):
        ...
        if item_type == 'label':
            ...
            return ['python', 'discover.py', item_value,
                    "%(item_dir)s/%(warc_file_base)s.txt.gz" % item, "en"]

        elif item_type == 'labelfr':
            # Expect something like label:1d38ed447cfccc87
            return ['python', 'discover.py', item_value,
                    "%(item_dir)s/%(warc_file_base)s.txt.gz" % item, "fr"]
        else:
...
project = Project(
    title="Baraza Discovery",
    project_html="""
        <img class="project-logo" alt="Project logo" src="http://archiveteam.org/images/4/40/Google_Logo.png" height="50px" title=""/>
        <h2>Baraza Phase 1.
        <span class="links">
             <a href="http://www.google.com/baraza/">Website</a> &middot;
             <a href="http://tracker.archiveteam.org/barazadisco/">Leaderboard</a>
             <a href="http://archiveteam.org/index.php?title=Google_Baraza">Wiki</a> &middot;
         </span>
        </h2>
        <p>Baraza is closing. This is phase 1: content discovery.</p>
    """,
...
pipeline = Pipeline(
    ...
    GetItemFromTracker("http://%s/%s" % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix="barazadisco"),
